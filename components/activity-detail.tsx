...
import { ArrowLeft, Clock3, HelpCircle, Play, Users } from "lucide-react";
  ...
  return <>
    ...
    <main className="detail-page compact-activity-detail"><div className="detail-shell">
      ...
      <article className="detail-card">
        ...
        <div className="compact-detail-meta"><span>{t("activity.level")} · <b>{levelLabel}</b></span><span><Clock3/>{activity.id === "debate-pros-cons" ? "20–40" : activity.id === "funny-questions" ? "15–40" : activity.id === "rank-it" ? "15–30" : activity.durationMinutes} min</span><span><Users/>{activity.groupSizes.join(", ")}</span>{activity.id === "funny-questions"&&<span>500 Questions</span>}</div>
        {activity.sourceType === "interactive" && <a className="button button-primary compact-start" href={activity.externalUrl}><Play/>{t("activity.startPractice")}</a>}
      </article>
      ...
      {["balance-game","rank-it","time-challenge","3-things-5-seconds","spyfall","secret-mission","never-have-i-ever","ice-breaking-3","choose-one-out-of-three","debate-pros-cons","useful-expressions","funny-questions"].includes(activity.id) ? <Link className="button button-primary compact-start" href={`/activities/${activity.id}/practice`}><Play/>{t("activity.startPractice")}</Link> : activity.sourceType === "internal" && <ActivityAccordion title={t("activity.startPractice")} openTitle={t("activity.closePractice")} icon={<Play/>}>{activity.id === "words-game" ? <AlphabetChallengeGame/> : <ConversationContentViewer activityId={activity.id}/>}</ActivityAccordion>}
