...
export const BALANCE_KEYS={state:"language101-balance-game-state",favorites:"language101-balance-game-favorites",sessions:"language101-balance-game-sessions",dataVersion:"balanceGameDataVersion"} as const;
...
const validBalanceIds=new Set(Array.from({length:350},(_,index)=>`balance-${String(index+1).padStart(3,"0")}`));
export const readBalanceFavorites=()=>{if(typeof window!=="undefined")localStorage.setItem(BALANCE_KEYS.dataVersion,"2");return read<string[]>(BALANCE_KEYS.favorites,[]).filter(id=>validBalanceIds.has(id))};
