...
export function loadSecretMissionSession(): SecretMissionSession | null {
  ...
  try {
    const value = JSON.parse(localStorage.getItem(SECRET_MISSION_SESSION_KEY) ?? "null") as SecretMissionSession | null;
...
export function saveSecretMissionSession(session: SecretMissionSession) {
  if (browser()) localStorage.setItem(SECRET_MISSION_SESSION_KEY, JSON.stringify(session));
}
