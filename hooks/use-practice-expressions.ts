...
import { getLocalDateKey,shiftDateKey } from "@/lib/daily-content";
...
export function usePracticeExpressions() {
  ...
  const [dateKey,setDateKey]=useState(() => getLocalDateKey()); const [record,setRecord]=useState<PracticeDayRecord>({date:dateKey,items:[],recommendationIds:[]}); const [ready,setReady]=useState(false);
  useEffect(() => { const timer=setTimeout(() => { setRecord(readPracticeDay(dateKey,language)); setReady(true); },0); return()=>clearTimeout(timer); },[dateKey,language]);
  useEffect(() => { const sync=(event:Event)=>{ const detail=(event as CustomEvent<PracticeDayRecord>).detail; if(detail?.date===dateKey)setRecord(detail);else if(event.type==="storage")setRecord(readPracticeDay(dateKey,language)); }; window.addEventListener("language101-practice-change",sync); window.addEventListener("storage",sync); return()=>{window.removeEventListener("language101-practice-change",sync);window.removeEventListener("storage",sync)}; },[dateKey,language]);
  useEffect(() => { const refresh=()=>{ const next=getLocalDateKey(); if(next!==dateKey)setDateKey(next); }; const midnight=new Date(`${shiftDateKey(dateKey,1)}T00:00:00`).getTime(); const timer=setTimeout(refresh,Math.max(250,midnight-Date.now()+100)); document.addEventListener("visibilitychange",refresh); return()=>{clearTimeout(timer);document.removeEventListener("visibilitychange",refresh);}; },[dateKey]);
  const persist=useCallback((next:PracticeDayRecord)=>{setRecord(next);writePracticeDay(next,language);window.dispatchEvent(new CustomEvent("language101-practice-change",{detail:next}));},[language]);
  ...
  const yesterday=readPracticeDay(shiftDateKey(dateKey,-1),language); const recent=useMemo(()=>ready?readRecentPracticeDays(dateKey,30,language):[],[dateKey,language,ready]);
