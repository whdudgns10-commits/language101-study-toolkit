...
export function usePracticeExpressions() {
  ...
  useEffect(() => { const sync=(event:Event)=>{ const detail=(event as CustomEvent<PracticeDayRecord>).detail; if(detail?.date===dateKey)setRecord(detail);else if(event.type==="storage")setRecord(readPracticeDay(dateKey)); }; window.addEventListener("language101-practice-change",sync); window.addEventListener("storage",sync); return()=>{window.removeEventListener("language101-practice-change",sync);window.removeEventListener("storage",sync)}; },[dateKey]);
