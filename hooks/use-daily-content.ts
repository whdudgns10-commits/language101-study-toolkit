...
const readState = (dateKey: string,language:string): DailyBrowserState => { try { const current=localStorage.getItem(dailyStorageKey(dateKey,language));const legacy=language==="en"?localStorage.getItem(dailyStorageKey(dateKey)):null;return { ...emptyState, ...JSON.parse(current||legacy||"{}") }; } catch { return emptyState; } };
const writeState = (dateKey: string,language:string,value: DailyBrowserState) => { localStorage.setItem(dailyStorageKey(dateKey,language), JSON.stringify(value)); window.dispatchEvent(new CustomEvent("language101-study-change")); };
const emptyExpression:Expression={id:"content-preparing",expression:"",koreanMeaning:"",example:"",exampleTranslation:"",usageTip:"",similarExpressions:[],level:"beginner",category:"conversation"};
...
export function useDailyContent() {
  ...
  const expressionPool=useMemo(()=>language==="en"?expressions.filter(item=>item.level==="beginner"):[],[language]);
  const primaryExpression = useMemo(() => expressionPool.length?selectDailyItem(expressionPool, dateKey, `${language}:daily-expression:beginner`, browserState.expressionOffset,30):emptyExpression, [dateKey,language,browserState.expressionOffset,expressionPool]);
  ...
  const yesterdayExpression = useMemo(() => language==="en"?selectDailyItem(expressions, shiftDateKey(dateKey, -1), `${language}:expression`):emptyExpression, [dateKey,language]);
