...
export function useDailyContent() {
  ...
  useEffect(() => { const sync=()=>setBrowserState(readState(dateKey)); window.addEventListener("language101-study-change",sync); window.addEventListener("storage",sync); return()=>{window.removeEventListener("language101-study-change",sync);window.removeEventListener("storage",sync)}; },[dateKey]);
  useEffect(() => {
  ...
  const primaryExpression = useMemo(() => selectDailyItem(expressions.filter(item=>item.level==="beginner"), dateKey, "daily-expression:beginner", browserState.expressionOffset,30), [dateKey, browserState.expressionOffset]);
  ...
  return { dateKey, primaryExpression, relatedExpressions, yesterdayExpression, dailyMissions, expressionUsed: browserState.expressionUsed, completedMissionIds: browserState.completedMissionIds,
    ...
    rerollMissions: () => { clearMissionProgress(dateKey);update({ ...browserState, missionOffset: browserState.missionOffset + 1, completedMissionIds: [] }); },
...
export function getDailyBrowserState(dateKey = getLocalDateKey()) { if (typeof window === "undefined") return emptyState; return readState(dateKey); }
