...
test("Random Activity candidates are unique, filterable, and exclude disabled entries",async()=>{const [rules,registry]=await Promise.all([import(new URL(`../lib/random-activity.ts?registry=${Date.now()}`,import.meta.url)),import(new URL(`../data/activities.ts?registry=${Date.now()}`,import.meta.url))]);const eligible=rules.uniqueActivities(registry.randomEligibleActivities);assert.equal(eligible.length,18);assert.equal(new Set(eligible.map(item=>item.id)).size,18);assert.equal(new Set(eligible.map(item=>item.slug||item.id)).size,18);for(const item of eligible)assert.ok(item.slug||item.id);});
...
test("Alphabet Challenge replaces the legacy activity while preserving its compatible slug",async()=>{
  ...
  assert.equal(registry.activeActivities.length,18);
...
test("all 18 activities have intentional unique icon mappings",async()=>{
  const registry=await import(new URL(`../data/activities.ts?icons=${Date.now()}`,import.meta.url));
  const expected={"true-or-false":"true-false","ice-breaking-3":"icebreaker","30-second-speaking":"timed-speaking","time-challenge":"time-challenge","20-questions":"questions","what-if-challenge":"imagination","funny-questions":"funny","fun-discuss":"discussion","guessing-words":"guessing","word-battle":"battle","balance-game":"balance","rank-it":"rank-it","conversation-starter":"conversation-starter","words-game":"alphabet","debate-pros-cons":"debate","choose-one-out-of-three":"choose-three","useful-expressions":"useful-expressions","describing-picture-game":"situation-story"};
  assert.equal(registry.activeActivities.length,18);
  assert.deepEqual(Object.fromEntries(registry.activeActivities.map(item=>[item.id,item.iconKey])),expected);
  assert.equal(new Set(registry.activeActivities.map(item=>item.iconKey)).size,18);
});

test("Balance Game is unified, data-rich, and keeps the shared activity registry",async()=>{const [registry,data]=await Promise.all([import(new URL(`../data/activities.ts?balance=${Date.now()}`,import.meta.url)),import(new URL(`../data/balance-game.ts?balance=${Date.now()}`,import.meta.url))]);assert.equal(registry.activeActivities.length,18);assert.equal(registry.activeActivities.filter(item=>item.id==="balance-game").length,1);assert.equal(registry.activeActivities.some(item=>item.id==="balance-game-2"),false);assert.equal(registry.randomEligibleActivities.length,18);assert.ok(data.balanceGameItems.length>=150);assert.equal(new Set(data.balanceGameItems.map(item=>item.id)).size,data.balanceGameItems.length);assert.ok(data.balanceGameItems.every(item=>item.optionA&&item.optionB&&item.followUpQuestions.en.length>=3));for(const level of ["beginner","intermediate","advanced"])assert.ok(data.balanceGameItems.filter(item=>item.level===level).length>=50);const pairs=data.balanceGameItems.map(item=>[item.optionA,item.optionB].map(v=>v.toLowerCase().trim()).sort().join("|"));assert.equal(new Set(pairs).size,pairs.length);});
...
test("Ice Breaking is second in the shared activity registry and has dedicated questions",async()=>{const [registry,data]=await Promise.all([import(new URL(`../data/activities.ts?ice=${Date.now()}`,import.meta.url)),import(new URL(`../data/ice-breaking-questions.ts?ice=${Date.now()}`,import.meta.url))]);assert.equal(registry.activeActivities.length,18);assert.deepEqual(registry.activeActivities.slice(0,2).map(item=>item.id),["true-or-false","ice-breaking-3"]);assert.equal(registry.activeActivities[1].title,"Ice Breaking");assert.equal(registry.randomEligibleActivities[1],registry.activeActivities[1]);assert.ok(data.iceBreakingQuestions.length>=150);assert.equal(new Set(data.iceBreakingQuestions.map(item=>item.id)).size,data.iceBreakingQuestions.length);for(const depth of ["easy","medium","deep"])assert.ok(data.iceBreakingQuestions.filter(item=>item.depth===depth).length>=50);});
...
test("Rank It is a first-class registry and Random Activity candidate",async()=>{const registry=await import(new URL(`../data/activities.ts?rank-it-registry=${Date.now()}`,import.meta.url));const item=registry.getActivity("rank-it");assert.ok(item);assert.equal(item.slug,"rank-it");assert.equal(item.sourceType,"internal");assert.equal(item.randomEligible,true);assert.equal(registry.activeActivities.length,18);assert.ok(registry.randomEligibleActivities.some(activity=>activity.id==="rank-it"));assert.equal(registry.activeActivities.indexOf(item),registry.activeActivities.findIndex(activity=>activity.id==="balance-game")+1)});
...
test("Time Challenge is registered as the eighteenth internal Random Activity",async()=>{const registry=await import(new URL(`../data/activities.ts?time-challenge=${Date.now()}`,import.meta.url));const item=registry.getActivity("time-challenge");assert.ok(item);assert.equal(item.slug,"time-challenge");assert.equal(item.sourceType,"internal");assert.equal(item.randomEligible,true);assert.equal(item.iconKey,"time-challenge");assert.equal(registry.activeActivities.length,18);assert.equal(registry.randomEligibleActivities.length,18);assert.ok(registry.randomEligibleActivities.includes(item))});
