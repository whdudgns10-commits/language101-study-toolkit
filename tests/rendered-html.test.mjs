...
test("Random Activity candidates are unique, filterable, and exclude disabled entries",async()=>{const rules=await import(new URL(`../lib/random-wheel.ts?registry=${Date.now()}`,import.meta.url));const items=Array.from({length:17},(_,index)=>({id:`activity-${index}`,title:`Activity ${index}`,enabled:true,randomEligible:true}));items.push({...items[0]});items.push({id:"disabled",title:"Disabled",enabled:false,randomEligible:true});items.push({id:"excluded",title:"Excluded",enabled:true,randomEligible:false});const eligible=rules.uniqueActivities(items);assert.equal(eligible.length,17);assert.ok(eligible.length>2);assert.equal(new Set(eligible.map(item=>item.id)).size,eligible.length);assert.equal(rules.selectWheelItems(eligible).length,12);});
...
test("Random Activity UI exposes filters, count, empty state, shuffle, localized labels and matching route",async()=>{const [wheel,localization,css,stats]=await Promise.all([readFile(new URL("../components/activity-wheel.tsx",import.meta.url),"utf8"),readFile(new URL("../lib/activity-localization.ts",import.meta.url),"utf8"),readFile(new URL("../app/globals.css",import.meta.url),"utf8"),readFile(new URL("../app/admin/content-stats/page.tsx",import.meta.url),"utf8")]);for(const contract of ["randomActivity.available","randomActivity.noMatches","shuffle","router.push","visualActivity.slug||visualActivity.id","SpinResult","RANDOM_HISTORY_KEY"])assert.match(wheel,new RegExp(contract.replace(/[|]/g,"\\|")));assert.match(localization,/translations\?\.\[language\]\|\|activity\.translations\?\.en/);assert.match(css,/wheel-filter-panel/);assert.match(css,/--wheel-font-size/);for(const label of ["전체 활성 활동","랜덤 선택 가능 활동","중복 slug","중복 id"])assert.match(stats,new RegExp(label));});

test("Random Activity waits for the result and automatically opens the matching activity",async()=>{const wheel=await readFile(new URL("../components/activity-wheel.tsx",import.meta.url),"utf8");assert.match(wheel,/onTransitionEnd=.*propertyName===\"transform\".*completeSpin\(selectedResult\)/);assert.match(wheel,/router\.push\(`\/activities\/\$\{targetSlug\}`\)/);assert.match(wheel,/disabled=\{spinning\|\|Boolean\(selectedResult\)\}/);assert.match(wheel,/clearTimeout\(redirectRef\.current\)/);assert.match(wheel,/clearTimeout\(spinFallbackRef\.current\)/);});
...
test("selected wheel result is the single source for display, history, and navigation",async()=>{const source=await readFile(new URL("../components/activity-wheel.tsx",import.meta.url),"utf8");assert.match(source,/const selectedIndex=pickWheelItemIndex\(items,history\)/);assert.match(source,/const selectedActivity=items\[selectedIndex\]/);assert.match(source,/document\.elementFromPoint\(hitX,hitY\)/);assert.match(source,/data-wheel-segment/);assert.match(source,/const visualActivity=wheelItems\[visualIndex\]/);assert.match(source,/itemsRef\.current/);assert.match(source,/setSelectedResult\(result\)/);assert.match(source,/nextRandomHistory\(history,result\.id\)/);assert.match(source,/RECENT_RANDOM_KEY,result\.id/);assert.match(source,/router\.push\(`\/activities\/\$\{targetSlug\}`\)/);assert.match(source,/Visual wheel mismatch detected/);assert.equal((source.match(/pickWheelItemIndex\(/g)||[]).length,1);});

test("all 17 wheel activities have readable short titles and compact browse links",async()=>{const [registry,browser]=await Promise.all([import(new URL(`../data/activities.ts?short=${Date.now()}`,import.meta.url)),readFile(new URL("../components/activity-browser.tsx",import.meta.url),"utf8")]);assert.equal(registry.activeActivities.length,17);for(const activity of registry.activeActivities){assert.ok(activity.shortTitle?.trim(),`missing shortTitle: ${activity.id}`);assert.doesNotMatch(activity.shortTitle,/\.\.\.|…/)}assert.match(browser,/activeActivities\.map/);assert.match(browser,/activity-list-button/);assert.doesNotMatch(browser,/description|searchQuery|filteredActivities/);});

test("wheel labels use at most two whole-word lines without ellipsis",async()=>{const [rules,css]=await Promise.all([import(new URL(`../lib/random-wheel.ts?labels=${Date.now()}`,import.meta.url)),readFile(new URL("../app/globals.css",import.meta.url),"utf8")]);for(const title of ["Debate Pros and Cons","Situation Sentence Game","Twenty Questions"]){const lines=rules.splitWheelTitle(title);assert.ok(lines.length>=1&&lines.length<=2);assert.equal(lines.join(" "),title)}assert.ok(rules.wheelFontSize(17)>=10);assert.ok(rules.wheelFontSize(17,true)>=9);const wheelCss=css.slice(css.indexOf(".wheel-section"));assert.doesNotMatch(wheelCss,/text-overflow:ellipsis/);assert.match(wheelCss,/\.activity-wheel span\.is-flipped>i\{transform:rotate\(180deg\)\}/);assert.match(wheelCss,/width:min\(92vw,520px\)/);});

test("wheel inverse angle resolves every intended segment including repeated spins",async()=>{const wheel=await import(new URL(`../lib/random-wheel.ts?inverse=${Date.now()}`,import.meta.url));for(const total of [2,5,12,17])for(let index=0;index<total;index++){const first=wheel.nextWheelRotation(0,index,total,6);const second=wheel.nextWheelRotation(first,index,total,6);assert.equal(wheel.selectedIndexFromRotation(first,total),index);assert.equal(wheel.selectedIndexFromRotation(second,total),index)}});

test("SVG wheel geometry maps all 17 visual segments back to the same activity",async()=>{const [geometry,registry,source]=await Promise.all([import(new URL(`../lib/wheel-geometry.ts?geometry=${Date.now()}`,import.meta.url)),import(new URL(`../data/activities.ts?geometry=${Date.now()}`,import.meta.url)),readFile(new URL("../components/activity-wheel.tsx",import.meta.url),"utf8")]);assert.equal(geometry.WHEEL_START_ANGLE,-90);assert.equal(geometry.POINTER_ANGLE,-90);assert.equal(registry.randomEligibleActivities.length,17);for(const [index,activity] of registry.randomEligibleActivities.entries()){const rotation=geometry.getRotationForIndex(index,17,6);assert.equal(geometry.getIndexAtPointer(rotation,17),index,activity.title);const visual=registry.randomEligibleActivities[geometry.getIndexAtPointer(rotation,17)];assert.equal(visual.slug||visual.id,activity.slug||activity.id);assert.match(geometry.getSegmentPath(index,17),/^M 50 50 L /)}for(const title of ["Fun Discuss","Balance Game","Balance Game 2","Word Battle","What If Challenge","Situation Sentence Game","20 Questions","Useful Expressions"])assert.ok(registry.randomEligibleActivities.some(item=>item.title===title),title);for(const attribute of ["data-wheel-segment","data-wheel-index","data-activity-id","data-activity-slug"])assert.match(source,new RegExp(attribute));});
