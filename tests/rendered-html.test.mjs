...
test("Topic Practice contains exactly one hundred complete validated topics",async()=>{
  const data=await import(new URL(`../data/topic-practice/2026-09-23-topic-practice-data.ts?topic-data=${Date.now()}`,import.meta.url));assert.equal(data.topicPracticeTopics.length,100);assert.deepEqual(data.validateTopicPracticeData(),[]);assert.deepEqual(data.topicPracticeCounts,{topics:100,warmups:500,questions:1000,vocabulary:1000,expressions:500,rolePlays:300,challenges:200});
  assert.equal(new Set(data.topicPracticeTopics.map(item=>item.id)).size,100);assert.equal(new Set(data.topicPracticeTopics.map(item=>item.title)).size,100);assert.ok(data.topicPracticeTopics.every(item=>item.vocabulary.every(value=>value.korean&&value.example)&&item.expressions.every(value=>value.korean&&value.example)&&item.rolePlays.every(value=>value.roleA&&value.roleB&&value.mission)));
  for(const title of ["Food","Travel","Dating","Job Interviews","Language Learning","Movies","Money","Gym","Working Holiday","Social Media","Korean Culture"])assert.ok(data.topicPracticeTopics.some(item=>item.title===title),title);
});

test("Topic Practice supports search, filters, favorites, recent-safe random, daily topic, cards, roles, and timers",async()=>{
  const [component,css]=await Promise.all([readFile(new URL("../components/2026-09-23-topic-practice.tsx",import.meta.url),"utf8"),readFile(new URL("../app/globals.css",import.meta.url),"utf8")]);for(const contract of ["Search topics...","topicPracticeCategories","language101-topic-practice-favorites","language101-topic-practice-recent","slice(0,10)","getTodayTopic","topic-practice","Favorite","RANDOM TOPIC","Previous","Shuffle","Next","Tap to see meaning","START ROLE PLAY","ROLE A","ROLE B","YOUR MISSION","setInterval","role=\"timer\""])assert.match(component,new RegExp(contract.replace(/[()+.*]/g,"\\$&")));
