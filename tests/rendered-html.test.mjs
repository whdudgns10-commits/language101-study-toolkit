...
test("Balance Game is unified, data-rich, and keeps the shared activity registry",async()=>{const [registry,data]=await Promise.all([import(new URL(`../data/activities.ts?balance=${Date.now()}`,import.meta.url)),import(new URL(`../data/balance-game.ts?balance=${Date.now()}`,import.meta.url))]);assert.equal(registry.activeActivities.length,22);assert.equal(registry.activeActivities.filter(item=>item.id==="balance-game").length,1);assert.equal(registry.activeActivities.some(item=>item.id==="balance-game-2"),false);assert.equal(registry.randomEligibleActivities.length,22);assert.ok(data.balanceGameItems.length>=150);assert.equal(new Set(data.balanceGameItems.map(item=>item.id)).size,data.balanceGameItems.length);assert.ok(data.balanceGameItems.every(item=>item.optionA&&item.optionB&&item.followUpQuestions.en.length>=3));for(const level of ["beginner","intermediate","advanced"])assert.ok(data.balanceGameItems.filter(item=>item.level===level).length>=50);const pairs=data.balanceGameItems.map(item=>[item.optionA,item.optionB].map(v=>v.toLowerCase().trim()).sort().join("|"));assert.equal(new Set(pairs).size,pairs.length);});
...
test("Conversation Starters replaces Ice Breaking with exactly 250 balanced questions",async()=>{const [registry,data]=await Promise.all([import(new URL(`../data/activities.ts?ice=${Date.now()}`,import.meta.url)),import(new URL(`../data/ice-breaking-questions.ts?ice=${Date.now()}`,import.meta.url))]);assert.equal(registry.activeActivities.length,22);assert.deepEqual(registry.activeActivities.slice(0,2).map(item=>item.id),["true-or-false","ice-breaking-3"]);assert.equal(registry.activeActivities[1].title,"Conversation Starters");assert.equal(registry.randomEligibleActivities[1],registry.activeActivities[1]);assert.equal(data.conversationStarters.length,250);assert.deepEqual(data.validateConversationStarters(),[]);assert.deepEqual(data.conversationStarters.map(item=>item.id),Array.from({length:250},(_,index)=>`conversation-starter-${String(index+1).padStart(3,"0")}`));assert.deepEqual(Object.values(data.conversationStarterCategoryCounts),[30,25,25,25,30,25,25,25,20,20]);assert.deepEqual(["easy","medium","deep"].map(level=>data.conversationStarters.filter(item=>item.difficulty===level).length),[100,100,50]);});
...
test("Conversation Starters supports filters, history, favorites, follow-ups, and safe v2 persistence",async()=>{const [detail,practice,page,css]=await Promise.all([readFile(new URL("../components/activity-detail.tsx",import.meta.url),"utf8"),readFile(new URL("../components/ice-breaking-practice.tsx",import.meta.url),"utf8"),readFile(new URL("../app/activities/[id]/page.tsx",import.meta.url),"utf8"),readFile(new URL("../app/globals.css",import.meta.url),"utf8")]);assert.match(detail,/ice-breaking-3.*practice/s);assert.match(page,/ice-breaking.*redirect\("\/activities\/ice-breaking-3"\)/s);for(const contract of ["conversationStarters[0].id","const previous","const next","const shuffle","slice(-RECENT_LIMIT)","conversationStartersDataVersion","language101-conversation-starters-favorites-v2","Favorites","Show Follow-up Questions","Everyone Answers","One Person Answers","Pair Discussion"])assert.match(practice,new RegExp(contract.replace(/[\[\].()]/g,"\\$&")));assert.match(css,/\.conversation-question-card/);assert.doesNotMatch(practice,/[가-힣]/);assert.doesNotMatch(await readFile(new URL("../data/ice-breaking-questions.ts",import.meta.url),"utf8"),/[가-힣]/);});
