...
test("Secret Mission is registered, renders, persists custom data, and protects private missions",async()=>{
  ...
  assert.equal(registry.activeActivities.length,21);
...
test("Random Activity candidates are unique, filterable, and exclude disabled entries",async()=>{const [rules,registry]=await Promise.all([import(new URL(`../lib/random-activity.ts?registry=${Date.now()}`,import.meta.url)),import(new URL(`../data/activities.ts?registry=${Date.now()}`,import.meta.url))]);const eligible=rules.uniqueActivities(registry.randomEligibleActivities);assert.equal(eligible.length,21);assert.equal(new Set(eligible.map(item=>item.id)).size,21);assert.equal(new Set(eligible.map(item=>item.slug||item.id)).size,21);for(const item of eligible)assert.ok(item.slug||item.id);});
...
test("all 21 activities have intentional unique icon mappings",async()=>{
...
test("Rank It is a first-class registry and Random Activity candidate",async()=>{const registry=await import(new URL(`../data/activities.ts?rank-it-registry=${Date.now()}`,import.meta.url));const item=registry.getActivity("rank-it");assert.ok(item);assert.equal(item.slug,"rank-it");assert.equal(item.sourceType,"internal");assert.equal(item.randomEligible,true);assert.equal(registry.activeActivities.length,21);assert.ok(registry.randomEligibleActivities.some(activity=>activity.id==="rank-it"));assert.equal(registry.activeActivities.indexOf(item),registry.activeActivities.findIndex(activity=>activity.id==="balance-game")+1)});
...
test("Time Challenge is registered as an internal Random Activity",async()=>{const registry=await import(new URL(`../data/activities.ts?time-challenge=${Date.now()}`,import.meta.url));const item=registry.getActivity("time-challenge");assert.ok(item);assert.equal(item.slug,"time-challenge");assert.equal(item.sourceType,"internal");assert.equal(item.randomEligible,true);assert.equal(item.iconKey,"time-challenge");assert.equal(registry.activeActivities.length,21);assert.equal(registry.randomEligibleActivities.length,21);assert.ok(registry.randomEligibleActivities.includes(item))});
