...
test("membership access tokens last exactly 24 hours and reject malformed values",async()=>{const access=await import(new URL(`../lib/2026-07-17-access-service.ts?access=${Date.now()}`,import.meta.url));const now=1_700_000_000_000;const token=access.createAccessToken(now);assert.equal(token.expiresAt-now,24*60*60*1000);assert.equal(access.isAccessTokenValid(JSON.stringify(token),now+1),true);assert.equal(access.isAccessTokenValid(JSON.stringify(token),token.expiresAt),false);assert.equal(access.isAccessTokenValid("invalid",now),false);assert.equal(access.STUDY_TOOLKIT_ACCESS_KEY,"studyToolkitAccess")});

test("every dedicated member practice route passes through the shared AccessGuard",async()=>{const routes=["balance-game","choose-one-out-of-three","debate-pros-cons","ice-breaking-3","word-battle"];for(const route of routes){const page=await readFile(new URL(`../app/activities/${route}/practice/page.tsx`,import.meta.url),"utf8");assert.match(page,/AccessGuard/,route);assert.match(page,/<AccessGuard>/,route)}const [detail,provider,guard,card,api,css]=await Promise.all([readFile(new URL("../components/activity-detail.tsx",import.meta.url),"utf8"),readFile(new URL("../components/access/2026-07-17-access-provider.tsx",import.meta.url),"utf8"),readFile(new URL("../components/access/2026-07-17-access-guard.tsx",import.meta.url),"utf8"),readFile(new URL("../components/access/2026-07-17-membership-access-card.tsx",import.meta.url),"utf8"),readFile(new URL("../app/api/membership/validate/route.ts",import.meta.url),"utf8"),readFile(new URL("../app/globals.css",import.meta.url),"utf8")]);assert.match(detail,/word-battle.*practice/s);assert.match(provider,/studyToolkitAccess|STUDY_TOOLKIT_ACCESS_KEY/);assert.match(guard,/membership-access\?next=/);assert.match(card,/Invalid Access Code/);assert.match(card,/Membership Expired/);assert.match(card,/101language\.base44\.app/);assert.match(api,/codeValid/);assert.match(api,/membership\.active/);assert.doesNotMatch(card,/0509/);assert.match(css,/prefers-color-scheme:dark/);assert.match(css,/\.membership-access-card/)});
