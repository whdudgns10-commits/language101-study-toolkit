...
test("Random Activity UI exposes filters, count, empty state, shuffle, localized labels and matching route",async()=>{const [wheel,localization,css,stats]=await Promise.all([readFile(new URL("../components/activity-wheel.tsx",import.meta.url),"utf8"),readFile(new URL("../lib/activity-localization.ts",import.meta.url),"utf8"),readFile(new URL("../app/globals.css",import.meta.url),"utf8"),readFile(new URL("../app/admin/content-stats/page.tsx",import.meta.url),"utf8")]);for(const contract of ["activities available","No activities match","shuffle","data-selected-id","selected.slug||selected.id","candidates.length===1","RANDOM_HISTORY_KEY"])assert.match(wheel,new RegExp(contract));assert.match(localization,/translations\?\.\[language\]\|\|activity\.translations\?\.en/);assert.match(css,/wheel-filter-panel/);assert.match(css,/text-overflow:ellipsis/);for(const label of ["전체 활성 활동","랜덤 선택 가능 활동","중복 slug","중복 id"])assert.match(stats,new RegExp(label));});
