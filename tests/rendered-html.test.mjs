...
test("Spyfall protects role flow and implements timer, private voting, reveal, recovery, and admin CRUD",async()=>{const [game,storage,admin,css]=await Promise.all([readFile(new URL("../components/2026-07-26-spyfall-game.tsx",import.meta.url),"utf8"),readFile(new URL("../lib/2026-07-26-spyfall-storage.ts",import.meta.url),"utf8"),readFile(new URL("../components/2026-07-26-spyfall-admin.tsx",import.meta.url),"utf8"),readFile(new URL("../app/globals.css",import.meta.url),"utf8")]);for(const contract of ["createSpyfallSpyNumbers","crypto.getRandomValues","phase === \"reveal\" ? \"handoff\"","roleIndex + 1 >= playerCount","setPhase(\"ready\")","timerRunning","setInterval","secondsLeft <= 10","new Audio","navigator.vibrate","beginVoting","voterIndex + 1 >= playerCount","setPhase(\"vote-summary\")","revealIdentity","spyGuessedLocation","resolveSpyfallWinner","resumeSnapshot"])assert.match(game,new RegExp(contract.replace(/[()+?.*]/g,"\\$&")));for(const key of ["language101-spyfall-settings","language101-spyfall-active-session","language101-spyfall-last-location","localStorage"])assert.match(storage,new RegExp(key));for(const contract of ["submitLocation","editLocation","submitQuestion","editQuestion","moveItem","active: !item.active","createDefaultSpyfallSettings","defaultMinutes","spyCounts"])assert.match(admin,new RegExp(contract.replace(/[?*+]/g,"\\$&")));for(const selector of [".spyfall-role.is-spy",".spyfall-role.is-citizen",".spyfall-player-grid",".spyfall-vote-bars",".spyfall-admin-list","safe-area-inset-bottom","prefers-reduced-motion"])assert.match(css,new RegExp(selector.replace(/[.()]/g,"\\$&")))});
