...
test("Ice Breaking is second in the shared activity registry and has dedicated questions",async()=>{const [registry,data]=await Promise.all([import(new URL(`../data/activities.ts?ice=${Date.now()}`,import.meta.url)),import(new URL(`../data/ice-breaking-questions.ts?ice=${Date.now()}`,import.meta.url))]);assert.equal(registry.activeActivities.length,22);assert.deepEqual(registry.activeActivities.slice(0,2).map(item=>item.id),["true-or-false","ice-breaking-3"]);assert.equal(registry.activeActivities[1].title,"Ice Breaking");assert.equal(registry.randomEligibleActivities[1],registry.activeActivities[1]);assert.ok(data.iceBreakingQuestions.length>=150);assert.equal(new Set(data.iceBreakingQuestions.map(item=>item.id)).size,data.iceBreakingQuestions.length);for(const depth of ["easy","medium","deep"])assert.ok(data.iceBreakingQuestions.filter(item=>item.depth===depth).length>=50);});
