...
test("Spyfall multi-mode JSON has twelve unique categories and at least 650 candidates",async()=>{const data=JSON.parse(await readFile(new URL("../data/2026-07-26-spyfall-categories.json",import.meta.url),"utf8"));assert.equal(data.length,12);assert.equal(new Set(data.map(item=>item.id)).size,12);assert.deepEqual(data.map(item=>item.id),["places","countries","jobs","movies","animals","foods","sports","celebrities","brands","history","space","school-subjects"]);assert.ok(data.every(item=>item.title&&item.titleKo));assert.ok(data.find(item=>item.id==="places").items.length>=100);assert.ok(data.filter(item=>item.id!=="places").every(item=>item.items.length>=50));assert.ok(data.reduce((sum,item)=>sum+item.items.length,0)>=650);for(const category of data)assert.equal(new Set(category.items.map(item=>item.toLowerCase())).size,category.items.length)});

test("Spyfall engine always includes the answer among twenty candidates and enforces round outcomes",()=>{const script=`import("./lib/2026-07-26-spyfall-engine.ts").then(engine=>{const candidateChecks=engine.spyfallCategories.map(category=>{const answer=category.items[0];const candidates=engine.buildSpyfallCandidates(category,answer,20,()=>0.42);return {length:candidates.length,unique:new Set(candidates).size,includes:candidates.includes(answer)}});console.log(JSON.stringify({errors:engine.validateSpyfallCategories(),candidateChecks,midSpy:engine.evaluateSpyfallVote(3,[3],"mid"),midCitizen:engine.evaluateSpyfallVote(2,[3],"mid"),finalCitizen:engine.evaluateSpyfallVote(2,[3],"final"),correctGuess:engine.evaluateSpyfallGuess(" South Korea! ","south korea"),wrongGuess:engine.evaluateSpyfallGuess("Japan","South Korea"),vote:engine.calculateSpyfallVote([2,2,3,2],[1,2,3,4])}))})`;const result=JSON.parse(execFileSync(process.execPath,["--import","tsx","-e",script],{cwd:new URL("..",import.meta.url),encoding:"utf8"}));assert.deepEqual(result.errors,[]);assert.ok(result.candidateChecks.every(check=>check.length===20&&check.unique===20&&check.includes));assert.deepEqual(result.midSpy,{winner:"citizens",eliminate:null,continueToRoundTwo:false});assert.deepEqual(result.midCitizen,{winner:null,eliminate:2,continueToRoundTwo:true});assert.deepEqual(result.finalCitizen,{winner:"spies",eliminate:null,continueToRoundTwo:false});assert.equal(result.correctGuess,"spies");assert.equal(result.wrongGuess,"citizens");assert.deepEqual(result.vote.leaders,[2]);assert.equal(result.vote.majority,true)});
...
test("Spyfall protects roles and implements rounds, chat logs, private voting, guesses, recovery, and admin CRUD",async()=>{const [wrapper,game,storage,admin,candidateGrid,playerBoard,votePanel,css]=await Promise.all([readFile(new URL("../components/2026-07-26-spyfall-game.tsx",import.meta.url),"utf8"),readFile(new URL("../components/spyfall/2026-07-26-spyfall-game.tsx",import.meta.url),"utf8"),readFile(new URL("../lib/2026-07-26-spyfall-storage.ts",import.meta.url),"utf8"),readFile(new URL("../components/2026-07-26-spyfall-admin.tsx",import.meta.url),"utf8"),readFile(new URL("../components/spyfall/2026-07-26-spyfall-candidate-grid.tsx",import.meta.url),"utf8"),readFile(new URL("../components/spyfall/2026-07-26-spyfall-player-board.tsx",import.meta.url),"utf8"),readFile(new URL("../components/spyfall/2026-07-26-spyfall-vote-panel.tsx",import.meta.url),"utf8"),readFile(new URL("../app/globals.css",import.meta.url),"utf8")]);assert.match(wrapper,/components\/spyfall\/2026-07-26-spyfall-game/);for(const contract of ["createSpyfallSpyNumbers","crypto.getRandomValues","phase === \"reveal\" ? \"handoff\"","roleIndex + 1 >= playerCount","setPhase(\"ready\")","timerRunning","setInterval","secondsLeft <= 10","new Audio","navigator.vibrate","beginVoting","voterIndex + 1 >= voters.length","setPhase(\"vote-summary\")","evaluateSpyfallVote","evaluateSpyfallGuess","startRoundTwo","saveQuestionLog","questionLogs","eliminatedPlayers","resumeSnapshot","SpyfallCandidateGrid","SpyfallPlayerBoard","SpyfallVotePanel"])assert.match(game,new RegExp(contract.replace(/[()+?.*]/g,"\\$&")));assert.match(storage,/version: 2/);for(const key of ["language101-spyfall-settings","language101-spyfall-active-session","language101-spyfall-last-location","categoryId","candidates","questionLogs","eliminatedPlayers","winnerReason","localStorage"])assert.match(storage,new RegExp(key));for(const contract of ["submitLocation","editLocation","submitQuestion","editQuestion","moveItem","active: !item.active","createDefaultSpyfallSettings","defaultMinutes","spyCounts"])assert.match(admin,new RegExp(contract.replace(/[?*+]/g,"\\$&")));assert.match(candidateGrid,/answer === candidate/);assert.match(playerBoard,/is-eliminated/);assert.match(votePanel,/Private vote/);for(const selector of [".spyfall-role.is-spy",".spyfall-role.is-citizen",".spyfall-candidate-grid",".spyfall-stage-bar",".spyfall-player-board",".spyfall-question-log",".spyfall-chat",".spyfall-modal-backdrop",".spyfall-vote-bars",".spyfall-admin-list","safe-area-inset-bottom","prefers-reduced-motion"])assert.match(css,new RegExp(selector.replace(/[.()]/g,"\\$&")))});
