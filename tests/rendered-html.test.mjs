...
test("Random Activity waits for the result and automatically opens the matching activity",async()=>{const wheel=await readFile(new URL("../components/activity-wheel.tsx",import.meta.url),"utf8");assert.match(wheel,/onTransitionEnd=.*completeSpin\(selectedResult,items\)/);assert.match(wheel,/router\.push\(`\/activities\/\$\{targetSlug\}`\)/);assert.match(wheel,/disabled=\{spinning\|\|Boolean\(selectedResult\)\}/);assert.match(wheel,/clearTimeout\(redirectRef\.current\)/);assert.match(wheel,/clearTimeout\(spinFallbackRef\.current\)/);});
