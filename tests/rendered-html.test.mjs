...
test("Balance Game is unified, data-rich, and keeps the shared activity registry",async()=>{const [registry,data]=await Promise.all([import(new URL(`../data/activities.ts?balance=${Date.now()}`,import.meta.url)),import(new URL(`../data/balance-game.ts?balance=${Date.now()}`,import.meta.url))]);assert.equal(registry.activeActivities.length,21);assert.equal(registry.activeActivities.filter(item=>item.id==="balance-game").length,1);assert.equal(registry.activeActivities.some(item=>item.id==="balance-game-2"),false);assert.equal(registry.randomEligibleActivities.length,21);assert.ok(data.balanceGameItems.length>=150);assert.equal(new Set(data.balanceGameItems.map(item=>item.id)).size,data.balanceGameItems.length);assert.ok(data.balanceGameItems.every(item=>item.optionA&&item.optionB&&item.followUpQuestions.en.length>=3));for(const level of ["beginner","intermediate","advanced"])assert.ok(data.balanceGameItems.filter(item=>item.level===level).length>=50);const pairs=data.balanceGameItems.map(item=>[item.optionA,item.optionB].map(v=>v.toLowerCase().trim()).sort().join("|"));assert.equal(new Set(pairs).size,pairs.length);});

test("Balance Game has a focused practice route, redirect, persistence, voting, and responsive cards",async()=>{const [page,detail,game,storage,css]=await Promise.all([readFile(new URL("../app/activities/[id]/page.tsx",import.meta.url),"utf8"),readFile(new URL("../components/activity-detail.tsx",import.meta.url),"utf8"),readFile(new URL("../components/balance-game.tsx",import.meta.url),"utf8"),readFile(new URL("../lib/balance-game-storage.ts",import.meta.url),"utf8"),readFile(new URL("../app/globals.css",import.meta.url),"utf8")]);assert.match(page,/balance-game-2.*redirect\("\/activities\/balance-game"\)/s);assert.match(detail,/`\/activities\/\$\{activity\.id\}\/practice`/);for(const contract of ["aria-pressed","setVotes","Reset Votes","readBalanceState","saveBalanceState","toggleBalanceFavorite","saveBalanceSession"])assert.match(game,new RegExp(contract));assert.match(storage,/normalizeBalanceActivityId/);assert.match(storage,/balance-game-2/);assert.match(css,/\.balance-option\{[^}]*min-height:190px/);assert.match(css,/@media\(max-width:700px\).*\.balance-options\{grid-template-columns:1fr/s);assert.match(css,/padding:18px 14px 110px/);});
