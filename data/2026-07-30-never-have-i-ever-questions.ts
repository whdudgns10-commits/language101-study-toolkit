...
const variants = [
  ...
  },
  {
    ...
    ko: (value: string) => `계획 없이 즉흥적으로 해본 경험: ${value}`,
  ...
  },
  {
    ...
    ko: (value: string) => `예상 밖의 결과로 이어진 경험: ${value}`,
  ...
  },
  {
    ...
    ko: (value: string) => `처음 만난 사람과 함께 해본 경험: ${value}`,
  ...
  },
  {
    ...
    ko: (value: string) => `다시 하고 싶었던 경험: ${value}`,
...
export const neverHaveIEverQuestions: NeverHaveIEverQuestion[] = Array.from({ length: 525 }, (_, index) => {
  ...
  return {
    ...
    category,
    english: `had the experience of ${variant.en(hint.en.replace(/^[A-Z]/, (letter) => letter.toLowerCase()))}.`,
    korean: `${variant.ko(hint.ko)}.`,
...
export function validateNeverHaveIEverQuestions() {
  ...
  if (new Set(neverHaveIEverQuestions.map((item) => item.english.toLowerCase())).size !== neverHaveIEverQuestions.length) errors.push("Duplicate English question.");
  if (neverHaveIEverQuestions.some((item) => !item.english || !item.korean || !item.difficulty || item.tags.length === 0 || !item.followUpEn || !item.followUpKo)) errors.push("Incomplete question.");
  return errors;
}
